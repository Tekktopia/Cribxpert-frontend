...
const SearchInput: React.FC<SearchInputProps> = ({
      ...
      if (searchQuery.trim() && !isSearching) {
        ...
        try {
          ...
          if (onSearch) {
            ...
            // Default behavior: navigate to discover page with search query
            navigate(
  ...
  const handleInputChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      // Show dropdown if there's a query and suggestions are enabled
  ...
  return (
    <form
      ...
      className={`flex-grow max-w-[500px] relative ${className}`}
    >
          ...
          {suggestions.map((suggestion, index) => (
            <button
              ...
              className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0 ${
                index === selectedSuggestionIndex ? 'bg-[#1d5c5c]/5' : ''
              }`}
            >
              <div className="text-[#1d5c5c] flex-shrink-0">
